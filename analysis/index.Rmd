...
Estimation of correlation matrix by shrinkage based approaches, in particular when the number of samples is smaller than the number of variables, is an ubiquitous problem in statistics. The popular shrinkage based methods however are not flexible in handling missing observations. Here, we propose an Empirical Bayes shrinkage approach called **CorShrink**, which adaptively learns how much to shrink the correlations by combining information across all pairs of variables, while also adjusting for the the varying amounts of missing data for each pair.

**CorShrink** is a very competing method to other correlation shrinkage approaches - PDSCE, corpcor, GLASSO etc under structured or sparse population covariance models. **CorShrink** is flexible to apply on correlation-like quantities such as cosine similarities between words from word2vec models, thereby generating more robust similairty scores. 
